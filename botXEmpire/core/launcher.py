...
from botXEmpire.utils.settings import config, logo
from botXEmpire.core.bot import run_bot
...
async def get_tg_clients(username) -> tuple[list[Client], dict]:
...
async def start() -> None:
	...
	log.info(f"Detected {len(get_session_names(username))} sessions | {len(get_proxies())} proxies")
	...
	if action == 1:
		...
		tg_clients, session_data = await get_tg_clients(username)
