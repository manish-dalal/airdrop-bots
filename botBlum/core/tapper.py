...
from botBlum.config import settings

from botBlum.utils import logger
from botBlum.exceptions import InvalidSession
...
class Tapper:
    ...
    def info(self, message):
        from botBlum.utils import info
        info(f"<blue>{self.session_name}</blue> | ℹ️ {message}")

    def debug(self, message):
        from botBlum.utils import debug
        debug(f"<light-yellow>{self.session_name}</light-yellow> | ⚙️ {message}")

    def warning(self, message):
        from botBlum.utils import warning
        warning(f"<light-red>{self.session_name}</light-red> | ⚠️ {message}")

    def error(self, message):
        from botBlum.utils import error
        error(f"<red>{self.session_name}</red> | 😢 {message}")

    def critical(self, message):
        from botBlum.utils import critical
        critical(f"<dark-red>{self.session_name}</dark-red> | 😱 {message}")

    def success(self, message):
        from botBlum.utils import success
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            self.start_param = random.choices([settings.REF_ID, 'ref_QRPckJZKX9'], weights=[75, 25], k=1)[0]
            ...
            while play_passes:
                ...
                if not game_id or game_id == "cannot start game":
                    logger.info(f"<light-yellow>{self.session_name}</light-yellow> | Couldn't start play in game!"
                                f" play_passes: <green>{play_passes}</green>, trying again")
                    tries -= 1
                    if tries == 0:
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            ...
            logger.info(f"<light-yellow>{self.session_name}</light-yellow> | Proxy IP: <green>{ip}</green>")
        except Exception as error:
        ...
        while True:
            try:
                ...
                if isinstance(play_passes, int):
                    self.info(f'You have <green>{play_passes}</green> play passes')
                ...
                if claim_amount != 0 and is_available:
                    ...
                    self.success(f"Claimed friend ref reward <yellow>{amount}</yellow> 🟡")
                ...
                try:
                    ...
                    if start_time is None and end_time is None:
                        ...
                        self.success(f"<lc>[FARMING]</lc> Claimed reward! Balance: <yellow>{balance}</yellow> 💰")
